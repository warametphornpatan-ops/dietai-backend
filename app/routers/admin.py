...
def register_admin(
    ...
    try:
        supabase.auth.admin.invite_user_by_email(
            payload.email.strip(),
            options={"redirect_to": "https://dietai-frontend.vercel.app/auth/set-password"}
        )
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"ส่งคำเชิญไม่สำเร็จ: {str(e)}")
...
def sync_admin_password(payload: SyncPasswordReq, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # ✅ FIX: รองรับทั้งแบบคืน list ตรง ๆ และแบบห่อใน .users
        users_list = supabase_users.users if hasattr(supabase_users, "users") else supabase_users
        ...
        # ปล่อย HTTPException ที่เราตั้งใจ raise ผ่านไปตรง ๆ
        raise
    except Exception as e:
